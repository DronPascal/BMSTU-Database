...
void chefInterface::addButtonClicked() {
    ...
    {
        qDebug() <<ingr<<" ";
        myQ.exec("UPDATE myschema.base SET Remaining = (SELECT Remaining FROM myschema.base WHERE Ingridients = '"+ingr+"') - ((SELECT "+ingr+" FROM myschema.recepies WHERE Shawarmas = '" +name+"') * " +count+ ") WHERE Ingridients = '"+ingr+"';");
//        myQ.exec("ALTER TABLE myschema.prom ADD "+ingr+" VARCHAR(30) DEFAULT 0");
//        myQ.exec("ALTER TABLE ADD CONSTRAINT "+ingr+"_check UNIQUE ("+ingr+") ");
    }
...
void chefInterface::onCustomContextMenu(const QPoint &point)
{
    ...
    if (selectedItem == removeAction)
    {
        ...
        QString name = orderModel->index(index.row(),0).data(Qt::DisplayRole).toString();
         QString count = orderModel->index(index.row(),2).data(Qt::DisplayRole).toString();
         qDebug()<< name  <<count;
         QStringList ingedients;
         myQ.exec("SELECT ingridients FROM myschema.base");
         while (myQ.next())
         ...
         {
qDebug() <<ingr;
             myQ.exec("UPDATE myschema.base SET remaining = (SELECT remaining FROM myschema.base WHERE ingridients = '"+ingr+"') + ("+count+" * (SELECT "+ingr+" FROM myschema.recepies WHERE shawarmas = '"+name+"')) WHERE ingridients = '"+ingr+"';");
         }
